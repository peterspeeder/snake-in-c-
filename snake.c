...
void init_snake() {
    ...
    new->y = rand() % (GRID_SIZE / 2) + (GRID_SIZE / 4); // Added random y position
    new->dir = SNAKE_UP;
    new->next = NULL;
...
void render_snake(SDL_Renderer *renderer, int x, int y) {
    ...
    SDL_Rect seg;
    seg.w = seg_size;
    seg.h = seg_size; // Corrected the variable name

    Snake *track = head;
    while (track != NULL) {
        seg.x = x + track->x * seg_size; // Corrected the assignment operator here
        seg.y = y + track->y * seg_size; // Corrected the assignment operator here
        SDL_RenderFillRect(renderer, &seg); // Corrected the function name here
        track = track->next;
    }
    return;
}
...
int main() {
    ...
    while (!quit) {
        ...
        render_snake(renderer, grid_x, grid_y); // Added rendering of the snake
